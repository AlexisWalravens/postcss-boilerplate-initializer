...
const cliRoot = path.resolve(new URL(currentFileUrl).pathname, '../../../');
const checkIfFileExists = async (fileName) => {
    const filePath = path.resolve(projectRoot, fileName);
    return await fs.pathExists(filePath);
};
const copyConfigFile = async (templateFileName, fileName, replaceArray) => {
...
const copyDirectory = async (destination) => {
    await fs.ensureDir(destination);
    const directoryToCopy = path.resolve(cliRoot, 'src/styles');
    try {
...
const addRequiredPackages = async ({ stylelint }) => {
...
const editPackageJson = async ({ stylelint }) => {
    const fileName = 'package.json';
    const packageJsonPath = path.resolve(projectRoot, fileName);
    try {
        const file = await fs.readJson(packageJsonPath);
        let generateScript = 'style-dictionary build --config ./style-dictionary.config.json';
        if (stylelint) {
...
const copyStyleDictionaryConfig = async (stylesDirectory) => {
    const templateFileName = 'style-dictionary.config.template.json';
    const fileName = 'style-dictionary.config.json';
    const replaceArray = [['%stylesDirectory%', stylesDirectory]];
    try {
...
const copyPostCSSConfig = async (stylesDirectory) => {
